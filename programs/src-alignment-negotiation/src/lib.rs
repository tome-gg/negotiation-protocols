...
pub mod src_alignment_negotiation {
    ...
    pub fn setup_negotation(ctx: Context<SetupNegotiation>, mentor: Pubkey) -> Result<()> {
        ctx.accounts.negotiation.start([ctx.accounts.apprentice.key(), mentor])
    }
    ...
    pub fn propose(ctx: Context<Negotiate>, proposal: Proposal) -> Result<()> {
        ...
        ctx.accounts.negotiation.negotiate(ctx.accounts.player.key(), &mut p)
    }
}
...
pub struct AlignmentNegotiation {
    ...
    proposal: Proposal,             // Proposal::MAXIMUM_SIZE
...
impl NegotiationState {
    ...
    fn update<T: Eq>(
        ...
        prev: T, 
        new: T) -> AlignmentResult<NegotiationState> {
        ...
        match event {
            ...
            NegotiationEvent::Propose => {
                ...
                Ok(NegotiationState::Proposed { proposer: (initiator) })
            }
            NegotiationEvent::Review => {
                ...
                Ok(NegotiationState::Reviewed { proposee: (initiator) })
            },
            NegotiationEvent::Accept => {
                ...
                Ok(NegotiationState::Accepted { proposee: (initiator) })
            },
...
impl AlignmentNegotiation {
    pub const MAXIMUM_SIZE: usize = (32 * 2) + 1 + 1 + 32 + 32 + 8 + (32 + 1) + (32 + 1) + (32 + 1) + (32 + 1) + 1 + (32 + 32 + 32 + 8 + 2 + 32 + 32);
    ...
    pub fn negotiate(&mut self, initiator: Pubkey, proposal: &mut Proposal) -> Result<()> {
        require!(self.is_negotiating(), AlignmentError::NegotiationAlreadyOver);
         ...
         */
        let term_result = self.term_state.update(initiator, proposal.get_term_event(), self.proposal.term, proposal.term);
        let new_term_state = match term_result {
            Ok(new_state) => new_state,
            Err(reason) => panic!("{}", reason)
        };

        let protocol_result = self.protocol_state.update(initiator, proposal.get_protocol_event(), self.proposal.protocol, proposal.protocol);
        let new_protocol_state = match protocol_result {
            Ok(new_state) => new_state,
            Err(reason) => panic!("{}", reason)
        };

        let parameters_result = self.parameters_state.update(initiator, proposal.get_parameters_event(), self.proposal.parameters, proposal.parameters);
        let new_parameters_state = match parameters_result {
            Ok(new_state) => new_state,
            Err(reason) => panic!("{}", reason)
        };

        let stake_result = self.stake_state.update(initiator, proposal.get_stake_event(), self.proposal.stakes, proposal.stakes);
        ...
        // Update negotiation
        self.proposal.term = proposal.term;
        self.proposal.protocol = proposal.protocol;
        self.proposal.parameters = proposal.parameters;
        self.proposal.stakes = proposal.stakes;
